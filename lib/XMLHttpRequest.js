...
function XMLHttpRequest(opts) {
  ...
  var assignProp = function (obj, key, value, assignable) {
    ...
    Object.defineProperty(obj, key, {
      ...
      set: assignable ? function (value) { dataMap[key] = value; } : undefined,
      enumerable: true,
      configurable: true
    });
    dataMap[key] = value;
  ...
  assignProp(this, 'send', function(data) {
    ...
    if (isLocal) {
      ...
      if (settings.async) {
        fs.readFile(fileURLToPath(url), function(error, data) {
          if (error) {
        ...
        try {
          dataMap.status = 200;
          const syncData = fs.readFileSync(fileURLToPath(url));
          // Use self.responseType to create the correct self.responseType, self.response.
          dataMap.responseURL = settings.url;
    ...
    if (settings.method === "GET" || settings.method === "HEAD") {
      ...
      if (data instanceof FormData) {
        const req = new Request(url.href, { method: settings.method, body: data });
    ...
    if (settings.async) {
      ...
      var createRequest = function (opt) {
        ...
        if (data) {
          if (data instanceof ReadableStream) {
            return stream.Readable.fromWeb(data).pipe(request);
          }
          request.write(data);
        }
      ...
      try {
        ...
        var contentFileEscaped = JSON.stringify(contentFile);
        var syncFile = path.join(tmpDir, ".node-xmlhttprequest-sync-" + process.pid);
        var syncFileEscaped = JSON.stringify(syncFile);
        ...
        var execString = "'use strict';"
          ...
          + "        fs.writeFileSync(" + contentFileEscaped + ", 'NODE-XMLHTTPREQUEST-ERROR-REDIRECT: Too many redirects', 'utf8');"
          + "        fs.unlinkSync(" + syncFileEscaped + ");"
          + "        return;"
          + "      }"
          + "      try {"
          ...
          + "      }"
          + "      catch (e) {"
          + "        fs.writeFileSync(" + contentFileEscaped + ", 'NODE-XMLHTTPREQUEST-ERROR-REDIRECT: Unsafe redirect', 'utf8');"
          + "        fs.unlinkSync(" + syncFileEscaped + ");"
          + "        return;"
          + "      };"
          + "      isSsl = url.protocol === 'https:';"
          ...
          + "      fs.writeFileSync(" + contentFileEscaped + ", JSON.stringify({err: null, data: { url: url.href, statusCode: response.statusCode, statusText: response.statusMessage, headers: response.headers }}), 'utf8');"
          + "      fs.writeFileSync(" + JSON.stringify(contentFile + '.bin') + ", responseData);"
          + "      fs.unlinkSync(" + syncFileEscaped + ");"
          + "    });"
          + "    response.on('error', function(error) {"
          + "      fs.writeFileSync(" + contentFileEscaped + ", 'NODE-XMLHTTPREQUEST-ERROR:' + JSON.stringify(error), 'utf8');"
          + "      fs.unlinkSync(" + syncFileEscaped + ");"
          + "    });"
